...
public static void main(String[] args) {
	...
	Map<String,Double> genderWiseSalary=new HashMap<>();
	
	for(Employee employee:allEmployees) {
		
		String gender=employee.getGender();
		
		if(genderWiseSalary.containsKey(gender)) {
			genderWiseSalary.put(gender,genderWiseSalary.get(gender)+employee.getBasic());
		}else {
			genderWiseSalary.put(gender, employee.getBasic());
		}
	}
	
	
	for(Map.Entry<String,Double>entry:genderWiseSalary.entrySet()) {
