...
public static void main(String[] args) {
	...
	SalesEmployee salesEmployee1 = new SalesEmployee(102, "Dinesh", "MALE", "Delhi", 1500.00, 100000.00);
	...
	for (int i = 1; i < n; i++) {
		...
		int j = i - 1;
		int r=salaryDescendingComparator.compare(employees[j], key);
		while (j >= 0 && r > 0) {
			employees[j + 1] = employees[j];
			j = j - 1;
			if(j>=0)
				r=salaryDescendingComparator.compare(employees[j], key);
		}
		employees[j + 1] = key;
