...
public static void main(String[] args) {
	Manager manager = new Manager(104, "Ram", "MALE", "Mumbai", 1500.00, 12);
	...
	Employee employee = new Employee(101, "Menaka", "FEMALE", "Cochin", 1400.0);
	...
	EmployeeBasicSalaryAscendingCompartor salaryAscendingCompartor = new EmployeeBasicSalaryAscendingCompartor();
	for (int i = 1; i < n; i++) {
		Employee key = employees[i];
		int j = i - 1;
		int r=salaryAscendingCompartor.compare(employees[j], key);
		while (j >= 0 && r > 0) {
			employees[j + 1] = employees[j];
			j = j - 1;
			if(j>=0)
				r=salaryAscendingCompartor.compare(employees[j], key);
		}
		employees[j + 1] = key;
